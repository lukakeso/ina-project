...
def get_genre(genre) :
    ...
    elif 'jazz' in genre or 'funk' in genre or 'big band' in genre or 'sax' in genre or 'bossa' in genre or 'blue' in genre or 'soul' in genre :
        simple_genre = 'jazz'
    ...
    with open('dataset/tracks.json', 'r', encoding='utf-8') as file:
        ...
        while i < n_of_songs: 
            ...
            if ': {' in line :
                ...
                danceability = float(line.split(':')[1].split(',')[0])
            elif 'energy"' in line:
                energy = float(line.split(':')[1].split(',')[0])
            elif 'loudness"' in line:
                loudness = float(line.split(':')[1].split(',')[0])
            elif 'tempo"' in line:
                tempo = float(line.split(':')[1].split(',')[0])
            elif 'valence"' in line:
                valence = float(line.split(':')[1].split(',')[0])
            elif 'acousticness"' in line:
                acousticness = float(line.split(':')[1].split(',')[0])
            elif 'liveness"' in line:
                liveness = float(line.split(':')[1].split(',')[0])
